...
const Publications = () => {
  ...
  const filtered = filter === "all" ? items : items.filter((p) => (p.type || "").toLowerCase() === filter.toLowerCase());
  ...
  return (
    <main className="section py-5">
      <Container>
        ...
        <div className="d-flex flex-wrap justify-content-center gap-2 mb-4" role="group" aria-label="Filter by type">
          {types.map((t) => (
            <Badge
              ...
              tabIndex={0}
              onClick={() => setFilter(t)}
              onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setFilter(t); } }}
            ...
            <Col key={p.id}>
              <Card className="h-100 shadow-sm">
                <Card.Body>
                  ...
                  <Card.Text className="small mb-1">{p.venue} {p.year != null && " (" + p.year + ")"}</Card.Text>
                  {p.abstract && <Card.Text className="small text-muted">{p.abstract}</Card.Text>}
                  <div className="mt-2">
                    {p.doi && <a href={"https://doi.org/" + p.doi} target="_blank" rel="noopener noreferrer" className="me-3 small">DOI</a>}
                    {p.url && <a href={p.url} target="_blank" rel="noopener noreferrer" className="small"><Icon icon="mdi:open-in-new" /> Link</a>}
                  </div>
                </Card.Body>
              </Card>
            </Col>
        ...
        {filtered.length === 0 && <p className="text-center text-muted">No publications in this category.</p>}
      </Container>
      <ScrollToTopButton />
    </main>
  );
